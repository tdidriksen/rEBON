...
feature -- Test, feature type
	...
	test_generic_feature_types_conform
		local
			bon_spec: BON_SPECIFICATION
		do
			set_up_test
			bon_spec := bon_specification_from_file ("feature_type_generic_conform.bon")
			assert ("Type checking succeeds", type_checker.check_bon_specification (bon_spec))
			if attached {TBON_TC_CLASS_TYPE} type_checker.type_with_name ("C", type_checker.formal_type_context) as class_type then
				assert ("Type C of feature id in class C conforms to type B of precursor",
						class_type.feature_with_name ("id", False, False) /= Void and then
	...
	test_aggregation_with_enclosing_class
		local
			bon_spec: BON_SPECIFICATION
		do
			set_up_test
			bon_spec := bon_specification_from_file ("aggregation_of_self.bon")
			assert_false ("Type checking fails because feature introduces aggregation with enclosing class", type_checker.check_bon_specification (bon_spec))
			assert ("Error: feature introduces aggregation with enclosing class", error_occurred (err_code_feature_introduces_aggregation_with_enclosing_class))
		end
end
