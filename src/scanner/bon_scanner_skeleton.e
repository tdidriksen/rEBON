...
feature {BON_PARSER} -- Mini-symbol table
	...
	resize_if_necessary (set: DS_SPARSE_SET[STRING])
	...
	is_extended_id_name (name: STRING): BOOLEAN
			-- Check if 'name' is a declared extended_id name.
		do
			Result := extended_id_names.has (name)
		end

	add_object_group_or_constant_name (name: STRING)
	...
	is_group_name (name: STRING): BOOLEAN
